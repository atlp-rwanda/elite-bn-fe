...
function Table() {
  ...
  const getData = async () => {
    ...
    setDataRows(
      requests && requests.data && requests.data.getAlltrips ? requests.data.getAlltrips : []
    )
      ...
      renderCell: (params) => {
        return (
          <Chip
            ...
            style={{
              backgroundColor:
                params.row.tripStatus == 'pending'
                  ? 'yellow'
                  : params.row.tripStatus == 'rejected'
                  ? 'red'
                  : 'green',
            }}
          />
  ...
  return (
    <div>
      ...
      <TableTemplate columns={columns} rows={dataRows} />
    </div>
  )
}
